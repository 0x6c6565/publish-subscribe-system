...
namespace PublishSubscribeSystem
{
    public class MessagePump
    {
        ...
        public void Subscribe<T>(ISubscriber<T> subscriber) where T : unmanaged, IEquatable<T>, IComparable<T>
        {
            if (!subscriptions.ContainsKey(typeof(T)))
            {
        ...
        public void Clear<T>() where T : unmanaged, IEquatable<T>, IComparable<T>
        {
            if (subscriptions.ContainsKey(typeof(T)))
            {
        ...
        public void UnsubscribeAll<T>() where T : unmanaged, IEquatable<T>, IComparable<T>
        {
            if (subscriptions.ContainsKey(typeof(T)))
            {
        ...
        public void Unsubscribe<T>(ISubscriber<T> subscriber) where T : unmanaged, IEquatable<T>, IComparable<T>
        {
            if (subscriptions.ContainsKey(typeof(T)))
            {
